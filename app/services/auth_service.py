...
from app.utils.helper import verify_password, create_token
...
class AuthServices(BasicServices):
    ...
    def create_tokens(self, user):
        ...
        try:
            ...
            self.logger.exception(f"Unexpected error during token generation for user: {user.username}: {e}")
    ...
    #     '''        
    #     try:
    #         self.logger.info(f"Refresh user token method started")
    ...
    #         self.logger.debug(f"Fetched token type and user_id from payload: {token_type}, {user_id}")
    #         if token_type != 'refresh':
    #             self.logger.debug(f"Token type is not refresh, token_type={token_type}")
    #             raise InvalidTokenException()

    #         self.logger.info(f"Fetching user by user_id of payload")
    #         user = super().get_record_by_id(uuid_user_id)

    #         self.logger.info(f"User fetched based on user_id in token")

    #         if not user.refresh_token == token:
    #             self.logger.info(f"Refresh token already invalidated")
    #             raise InvalidatedRefreshTokenException()
    #         return user
    ...
    #         self.db.commit()
    #         self.logger.info(f"Refresh token invalidated")
